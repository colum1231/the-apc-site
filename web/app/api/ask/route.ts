...
export async function POST(req: Request) {
  try {
    ...
    if (!apiKey || !projectId) return NextResponse.json({ error: "Missing server envs" }, { status: 500 });
    ...
    if (!convRes.ok) return NextResponse.json({ error: await convRes.text() }, { status: 502 });
    ...
    if (!msgRes.ok) return NextResponse.json({ error: data?.message || "CustomGPT error" }, { status: 502 });
