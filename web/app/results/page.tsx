...
// Reads searchParams on the server and calls CustomGPT directly.
...
async function askCustomGPT(prompt: string) {
  ...
  if (!apiKey || !projectId) {
    return { error: "Missing server envs" };
  }
  ...
  if (!convRes.ok) {
    ...
    return { error: `Conversation error: ${t}` };
  }
  ...
  if (!msgRes.ok) {
    return { error: data?.message || "CustomGPT error" };
  }
