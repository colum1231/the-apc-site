    ...
    fetchCustomGPTResults(q)
      .then((res) => {
        ...
        if (res?.data?.openai_response) {
          try {
            // Remove code block markers and trailing text
            let jsonPart = res.data.openai_response;
            jsonPart = jsonPart.replace(/^```json\s*/i, '').replace(/```$/g, '');
            jsonPart = jsonPart.split('— APC Almanac')[0].trim();
